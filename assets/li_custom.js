document.addEventListener("alpine:init", () => {
  Alpine.data(
    ...
    (
      ...
      formSelector:
        "#product_form_" +
        (pProduct?.id ? pProduct.id : selectedOrFirstAvailable?.id),
      ...
      get variantsBasedOnFilter() {
        ...
        if (Object.keys(this.filter).length === 0) {
          ...
          result.availableVariants = this.product?.variants.filter(
            (variant) => {
              ...
              return Object.entries(filter).every(
                ([filterName, filterValue]) => {
                  console.log(
                    "filterValue",
                    filterValue,
                    this.getFirstTwoSelectedFilters()
                  );
                  const matchingOption = Object.entries(variant).find(
                    ([key, value]) =>
        ...
        result.existingOptions = this.getExistingOptions(
      ...
      observeVariants() {
        if (this.isProductPage) {
          ...
          window.onpopstate = function (event) {
            ...
            selectedOrFirstAvailable = $this?.product.variants.filter(
            ...
            $this.product.options.forEach(function (option) {
              let selector =
                " [data-productVariantsLoop] input[name='" + option + "']";
              if ($this.selectorType === "select") {
                selector =
                  " [data-productVariantsLoop] select[name='" +
                  option +
                  "'] option";
              }
              document
                .querySelectorAll($this.formSelector + selector)
                .values()
      ...
      updateVariantStatuses() {
        ...
        const selectedOptionOneVariants = this.product.variants.filter(
          (variant) => {
            let element = form?.querySelector(
              '[data-productVariantsLoop] input[type="radio"]:checked'
            );
            if (this.selectorType === "select") {
              element = form?.querySelector(
                "[data-productVariantsLoop] select"
              );
            }
        ...
        if (form?.querySelectorAll("[data-ProductVariantsLoop]")) {
          inputWrappers = [
            ...form.querySelectorAll("[data-ProductVariantsLoop]"),
          ];
        }
        ...
        inputWrappers?.forEach((option, index) => {
          ...
          let previousOptionSelected =
            inputWrappers[index - 1].querySelector(":checked");
          if (previousOptionSelected?.getAttribute("data-disabled") !== null) {
            previousOptionSelected = inputWrappers[index - 1].querySelector(
              "input:not([data-disabled]), option:not([data-disabled])"
            );
          }
          ...
          if (this.selectorType === "select") {
            ...
            for (
              let i = 0;
              i < option.querySelector("select").options.length;
              i++
            ) {
              // Select the next available option if not in available options
              if (
                availableOptionInputsValue.indexOf(selectedVal) === -1 &&
                option.querySelector("select").options[i].text ===
                  availableOptionInputsValue[0]
              ) {
      ...
      activateVariants() {
        if (
          this.$el.querySelectorAll('[li-element="product-option-select"]')
            .length > 0
        ) {
        ...
        checkboxes.forEach((checkbox) => {
          checkbox.addEventListener("change", function () {
            this.parentNode.parentNode
              .querySelectorAll("[li-element=product-option-input]")
              .forEach((element) => {
        ...
        this.product.variants.map((variant) => {
          if (
            this.firstOptionHasAvailableChildren[variant?.option1] === undefined
          ) {
            this.firstOptionHasAvailableChildren[variant?.option1] = 0;
          }
          this.firstOptionHasAvailableChildren[variant?.option1] +=
            variant?.available;
        });
      ...
      getVariantElement(optionName) {
        ...
        let element = form?.querySelector(
          "[data-productVariantsLoop] input[name='" + optionName + "']:checked"
        );
        if (element && element?.getAttribute("data-disabled") !== null) {
          element = form.querySelector(
            "[data-productVariantsLoop] input[name='" +
              optionName +
              "']:not([data-disabled])"
          );
        }
      ...
      filterVariants() {
        ...
        let selectedOptions = [option1?.value, option2?.value, option3?.value];
      ...
      findAvailableVariant() {
        ...
        if (matchedProducts.length < 1) {
          console.info(
            "Liquify - No next best option found, using initial first available variant"
          );
      ...
      setVariant(pushState = true) {
        ...
        this.product.selected_or_first_available_variant.featured_image =
      ...
      async addToCartWithBundle(toggleMiniCart = true) {
        ...
        products.push({
          id:
            ...
            this.product.selected_or_first_available_variant?.id,
          quantity:
            this.product.selected_or_first_available_variant.quantity ?? 1,
        });
        ...
        try {
          ...
          console.log(
            "Liquify - Items added (addToCartWithBundle()):",
            data.items
          );
      ...
      async addToCart(
        ...
        e.preventDefault();
        console.log(
          "Liquify - Add to cart called, selected_or_first_available_variant.id:",
          this.product.selected_or_first_available_variant?.id
        );
      ...
      async addToCartAndCheckout(
        ...
        await this.addToCart(e, toggleMiniCart, productId, quantity).then(
          () => {
  ...
  Alpine.data("handleGlobalBundle", (useQuantity = true, shopMoneyFormat) => ({
    ...
    async addGlobalBundleToCart() {
      ...
      await this.callAddToCartApi(body).then((response) => {
    ...
    addOrIncreaseGlobalBundleItem(item) {
      let globalBundle = this.getGlobalBundleFromLocalStorage();
      let existingItem = globalBundle.find(
        (bundleItem) => bundleItem.variant.id === item.variant.id
      );
    ...
    updateQuantityOfBundleItem(bundleItemId, quantity) {
      ...
      let bundleItem = bundle.find(
        (bundleItem) => bundleItem.id === bundleItemId
      );
    ...
    updateBundleIndex() {
      let globalBundleIndex = parseInt(
        localStorage.getItem("globalBundleIndex")
      );
      globalBundleIndex += 1;
  ...
  // Delegated click handlers for add-to-cart and checkout-add-to-cart
  document.addEventListener("click", (event) => {
    const el = event.target.closest(
      '[li-element="add-to-cart"], [li-element="checkout-add-to-cart"]'
    );
    if (!el) return;

    event.preventDefault();

    const isCheckout = el.getAttribute("li-element") === "checkout-add-to-cart";

    const form = el.closest("form");
    let quantity = 1;
    let quantityEl = form
      ? form.querySelector('[li-element="add-to-cart-quantity"]')
      : null;
    if (!quantityEl && form) {
      quantityEl = form.querySelector('[name="quantity"]');
    }

    if (quantityEl && quantityEl.value) {
      const parsed = parseInt(quantityEl.value, 10);
      if (!isNaN(parsed) && parsed > 0) quantity = parsed;
    } else if (el.dataset.quantity) {
      const parsed = parseInt(el.dataset.quantity, 10);
      if (!isNaN(parsed) && parsed > 0) quantity = parsed;
    }

    const toggleAttr = el.getAttribute("data-toggle-minicart");
    const toggleMiniCart = toggleAttr === null ? true : toggleAttr !== "false";
    ...
    if (!componentData) {
      console.error(
        "Liquify - No Alpine component context found for add-to-cart action"
      );
      return;
    }
    ...
    if (
      isCheckout &&
      typeof componentData.addToCartAndCheckout === "function"
    ) {
      componentData.addToCartAndCheckout(
        event,
        toggleMiniCart,
        undefined,
        quantity
      );
    } else if (typeof componentData.addToCart === "function") {
      componentData.addToCart(event, toggleMiniCart, undefined, quantity);
    } else {
      console.error(
        "Liquify - Expected addToCart/addToCartAndCheckout not found on Alpine component"
      );
    }
  });
});
